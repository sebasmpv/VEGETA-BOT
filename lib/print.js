...
console.log(`
╭━━━━━━━━━━━━━━𖡼
┃ » ${chalk.white.bold('EliteBot:')} ${chalk.cyan.bold('%s')} 
...
┃ » ${chalk.white.bold(mid.idioma_code === 'es' ? 'Usuario:' : 'User:')} ${chalk.white('%s')} / ${chalk.bgMagentaBright.bold(user.role.replace(/\*/g, ''))}  ${user?.premiumTime > 0 ? '' : ''}
...
//╰────────────✦`.trim(),            
me + (conn.user.name == undefined ? '' : ' ~' + conn.user.name) + `${conn.user.jid == global.conn.user.jid ? '' : ' ProyectoX'}`,
